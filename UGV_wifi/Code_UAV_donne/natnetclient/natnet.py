...
class NatCommSocket(NatSocket):
    ...
    def recv(self):
        ...
        logging.info("[Client] Received command from %s: Command=%d, nDataBytes=%d",self.client_ip, packet.iMessage, packet.nDataBytes)
        return packet
    ...
    def send(self, nat_value, message='', sleep_time=.02):
        ...
        if message_len > 0:
            self._sock.sendto(pack("HH"+str(message_len+1)+"s", nat_value, 4+message_len, message), (self.server_ip, self.port))  # send both nat_value and packet size
        else:
        ...
        for send_attempt in range(num_attempts):
            ...
            for receive_attempt in range(num_attempts):
                ...
                if packet.iMessage == nat_value + 1:
                    logging.info("Correct packet received on attempt %d.",receive_attempt)
...
class NatClient(object):
    ...
    def ping(self):
        ...
        logging.info('Ping response received from compatible server.')
    ...
    def set_take_file_name(self, file_name):
        ...
        self.comm_sock.send(NAT_REQUEST, "SetRecordTakeName," + str(file_name))
    ...
    def start_recording(self, n_attempts=3):
        ...
        for attempt in range(n_attempts):
            ...
            self.comm_sock.send(NAT_REQUEST, b'StartRecording')
    ...
    def stop_recording(self, n_attempts=3):
        ...
        # Send "StopRecording" command to NatNet via a NATREQUEST command
        for attempt in range(n_attempts):
            self.comm_sock.send(NAT_REQUEST, b'StopRecording')  # Using comm_sock.get_data sometimes gets annoying non-blocking error.
            ...
            if self.is_recording == False:
                ...
                logging.info("Recording failed on attempt number %d.  Trying again...",attempt)
    ...
    def get_data(self):
        ...
        for el in range(nRigidBodies):  # nRigidBodies
            ...
            # Dirty change of reference frame
            body.position = z, -x, -y
            body.quaternion = qz, -qx, -qy, qw
        ...
        # Final Frame Info
        self.latency = unpack('f', data.read(4))[0]  # Deprecated value
        (timecode, timecodeSub) = unpack('2I', data.read(8))  # Requires optitrack esync HUB
    ...
    def wait_for_recording_start(self, debug_mode=False):
        ...
        if not debug_mode:
            logging.info("Waiting for recording to begin...")
    ...
    def wait_for_recording_stop(self, debug_mode=False):
        ...
        if not debug_mode:
            logging.info("Waiting for recording to end...")
